        ...
        holder.btn_follow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (holder.btn_follow.getText().toString().equals("팔로우")) {
                    ...
                    FirebaseDatabase.getInstance().getReference()
                            .child("Follow").child(user.getId()).child("followers").child(firebaseUser.getUid()).setValue(true);
                } else {
                    FirebaseDatabase.getInstance().getReference()
                            .child("Follow").child(firebaseUser.getUid()).child("following").child(user.getId()).removeValue();
                    FirebaseDatabase.getInstance().getReference()
                            .child("Follow").child(user.getId()).child("followers").child(firebaseUser.getUid()).removeValue();
                }
            }
        });
    }
        ...
        reference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.child(userid).exists()) {
                    button.setText("팔로잉");
                    button.setBackgroundResource(R.drawable.followbtn_normal);
                    button.setPadding(10, 10, 10, 10);
                } else {
                    button.setText("팔로우");
                    button.setBackgroundResource(R.drawable.followbtn_ok);
                    button.setPadding(10, 10, 10, 10);
                }
            }
